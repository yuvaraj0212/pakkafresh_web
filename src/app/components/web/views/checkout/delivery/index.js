...
export default class Deliverydetails extends Component {
    ...
    async componentDidMount() {
        ...
        let location = await GetLocationDetails.getLocationListDetails(token);
        if (location) {
            this.setState({ locationList: location.result })
        } else {
        ...
        return (
            <div className="card-body">
                <form onSubmit={this.handleSubmit}>
                    ...
                    <button type="submit" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree" className="btn btn-secondary mb-2 btn-lg" onClick={this.handleSubmit}>NEXT</button>
                </form>
            </div>
        )
    }
